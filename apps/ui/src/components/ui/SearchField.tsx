...
export function SearchField(
  ...
  return (
    <AriaSearchField {...props} className={composeTailwindRenderProps(props.className, 'group flex flex-col gap-1 min-w-[40px] font-sans max-w-full')}>
      ...
      <FieldGroup>
        <IconSearch aria-hidden className="w-4 h-4 ml-2 text-neutral-500 dark:text-neutral-400 forced-colors:text-[ButtonText] group-disabled:text-neutral-200 dark:group-disabled:text-neutral-600 forced-colors:group-disabled:text-[GrayText]" />
        ...
        <FieldButton className="mr-1 w-6 group-empty:invisible">
          <IconX aria-hidden className="w-4 h-4" />
        </FieldButton>
      </FieldGroup>
