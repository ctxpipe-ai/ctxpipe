...
import { FileText, Folder } from "fumadocs-ui/internal/icons"
...
export function DocsSidebarModeLinks() {
  ...
  const selfHostingActive = isSelfHostingPath(pathname)
  const docsActive = isDocsPath(pathname) && !selfHostingActive

  return (
    <div
      className={cn(
        "docs-mode-links mb-3 min-h-28 border-b border-fd-border",
        /* Center the two rows in the band between the chrome above and the tree below */
