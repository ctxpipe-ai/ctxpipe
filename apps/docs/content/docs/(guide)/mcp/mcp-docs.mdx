...
npx ctxpipe doctor mcp --url "https://app.ctxpipe.ai/mcp?orgSlug=acme"
```
...
Start with the lightweight doctor before opening a client or MCPJam session.
See [MCP troubleshooting](/docs/mcp/troubleshooting) for the full workflow:
local doctor, PR preview, production test organisation, then version-pinned
MCPJam for OAuth and `ctx_advisor`.

A `ready-for-oauth` doctor result means discovery is coherent. It does not prove
that authenticated tools work.

```bash
npx ctxpipe doctor mcp --url "https://app.ctxpipe.ai/mcp?orgSlug=your-org"
```
...
- **Your client cannot complete OAuth** - use a client with remote MCP OAuth support. The hosted docs assume OAuth-based MCP access. Follow [MCP troubleshooting](/docs/mcp/troubleshooting) if discovery itself is failing.
...
- [MCP troubleshooting](/docs/mcp/troubleshooting) - doctor mcp, OAuth discovery, and MCPJam hand-off
