...
`?orgSlug=your-org` is still valid and required only when the signed-in user
belongs to several organizations and has no active organization. For
self-hosted deployments, keep the same `/mcp` path and swap in your own base
URL.
...
- `orgSlug` is optional when the signed-in user has one organization, or an
  active organization on the session. Replace `your-org` with your organization
  slug when you pass it explicitly.
