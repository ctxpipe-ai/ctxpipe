...
Keys authenticate MCP with the **`x-api-key`** header, or as **`Authorization: Bearer <api-key>`** when a host cannot set custom headers (for example CodeRabbit). Prefer `x-api-key` when the client supports it. OAuth access tokens continue to use Bearer as well; the server distinguishes opaque/JWT OAuth tokens from API keys.
...
If a host lets you attach a custom request header, send **`x-api-key`** interpolating `CTXPIPE_API_KEY`. If the host only supports Bearer credentials, send `Authorization: Bearer` with the same key value. Do not put the secret in client or repository files.
