...
The CLI can also set up **local memory**: Markdown under `.ai/memory/` that
coding agents read and add to as they learn how your team works. It is **not** a
second MCP server. Hooks only save candidates to a gitignored inbox; agents keep
what matters, you commit it with the work, and merged lessons reach ctx|.
