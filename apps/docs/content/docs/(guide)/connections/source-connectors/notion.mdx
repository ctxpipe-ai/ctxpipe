...
<Callout title="Fully managed app flow">
  ...
  configuration. Self-hosted operators register a public integration in the
  connector setup wizard (client ID, secret, and Event URL). Setting
...
<Steps>
  <Step>
    ...
    **Register Notion integration** first, then authorize the workspace in the
    Notion popup and share the pages or databases the connector should be
    allowed to read.
  </Step>
...
### Authorization fails
    ...
    support. On a self-hosted deployment, finish **Register Notion integration**
    in the wizard (or the optional env shortcut) and confirm the callback URI
    matches exactly.
