...
<Callout title="Fully managed app flow">
  ...
  configuration. Self-hosted operators register a private OAuth app in the
  connector setup wizard (client ID, secret, then Add webhook). Setting
  `NOTION_*` environment variables is an optional operator shortcut, not a
  required restart. See [Self-host Notion](/docs/self-hosting/notion).
</Callout>
...
<Steps>
  <Step>
    ...
    On a self-hosted deployment without shared env credentials, complete
    **Register Notion OAuth app** and **Add webhook** first, then authorize the
    workspace in the Notion popup and share the pages or databases the connector
...
### Authorization fails
    ...
    support. On a self-hosted deployment, finish **Register Notion OAuth app**
    and **Add webhook** in the wizard (or the optional env shortcut) and confirm
