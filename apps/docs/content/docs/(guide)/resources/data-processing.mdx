...
- Connected Linear, Notion, Confluence, and captured Slack content is written
  into a Git repository you control, then follows the same downstream indexing
  and graph path as other repository content.
...
- You can **remove repositories** at any time; that stops further ingestion for those sources (existing indexed data may remain until your organization’s deletion processes complete - [get in touch](/docs/getting-started/get-in-touch) for specifics).
...
| Provider    | Purpose                 |
| ----------- | ----------------------- |
| Railway     | Compute, hosting, and our self-hosted observability (logs, traces, LLM traces) |
...
We do **not** publish fixed customer-data retention periods in this documentation yet. Session and auth artifacts in the app use **short-lived tokens** (for example password-reset links and invitations) with server-side expiry; those are account mechanics, not a statement about how long code index or graph data is retained. For deletion timelines, export, or DPA questions, **[get in touch](/docs/getting-started/get-in-touch)**.

## What we don’t do

- We **don’t** use your repository content to **train** ctx|’s own models (ctx| is not a model trainer).
- On the hosted product, every model call goes only to **[Zero Data Retention (ZDR)](https://openrouter.ai/docs/guides/features/zdr)** endpoints: ZDR is enforced in our OpenRouter account, so providers neither store your prompts and context nor train on them.
...
Questions about your organization’s data, subprocessors, or contracts: **[Get in touch](/docs/getting-started/get-in-touch)**.
