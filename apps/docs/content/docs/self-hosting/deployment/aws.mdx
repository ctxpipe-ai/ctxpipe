...
Leave `otel` unset and the tasks export nothing. There is no bundled collector. Set one OTLP/HTTP base URL on `endpoint`; the construct appends `/v1/traces`, `/v1/logs`, and `/v1/metrics` and sets `OTEL_SERVICE_NAME` to `backend`, `openworkflow`, `ui`, or `codesearch` on that task.
...
| Prop | Env on backend, worker, UI, and codesearch |
|---|---|
| `endpoint` | `OTEL_EXPORTER_OTLP_{TRACES,LOGS,METRICS}_ENDPOINT` |
| `headers` | `OTEL_EXPORTER_OTLP_HEADERS` (Secrets Manager) |
| `resourceAttributes` | `OTEL_RESOURCE_ATTRIBUTES` |

Browser telemetry and the signal URLs the apps read are in [Configuration](/docs/self-hosting/configuration). Prop reference: [package README](https://github.com/ctxpipe-ai/ctxpipe/blob/main/packages/aws-cdk/README.md#observability).
