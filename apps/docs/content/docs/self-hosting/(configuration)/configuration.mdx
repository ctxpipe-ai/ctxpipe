...
PagerDuty uses a deployment-owned OAuth application. After a user authorizes
an account, ctx| creates the signed webhook subscription. Put the OAuth
credentials on both backend and worker services.

| Variable | Description |
|---|---|
| `PAGERDUTY_CLIENT_ID`, `PAGERDUTY_CLIENT_SECRET` | PagerDuty OAuth application credentials. Both are required to enable the connector. |
