...
When **`MODEL_PROVIDER=openrouter`** (or hosted deployments that set it), the **medium** tier sends OpenRouter’s `models` array so that if the primary model errors (rate limits, downtime, moderation), routing tries **`MODEL_FAST_NAME`** next, then **`MODEL_HIGH_NAME`** — see [OpenRouter model fallbacks](https://openrouter.ai/docs/guides/routing/model-fallbacks). With defaults that is medium → Gemini 3 Flash → Kimi K2.6. This does **not** apply to `openai-like`.
