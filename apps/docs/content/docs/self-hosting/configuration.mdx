...
|---|---|
| `DATABASE_URL` | Runtime PostgreSQL URL for backend, worker, and codesearch. Must be the `ctxpipe_app` role (no `BYPASSRLS`). Example: `postgresql://ctxpipe_app:pass@host:5432/db`. Migrate uses the table-owner URL instead. **AWS CDK** operators do not set either URL — `cdk deploy` writes them. |
...
Self-hosted Slack uses one deployment-owned Slack app. Organisations authorise
...
remain usable by background sync jobs.
...
Registry. Runtime services (`backend`, `worker`, `codesearch`) must use a
`DATABASE_URL` for `ctxpipe_app`. The migrate job uses the Neon owner / pooler
URL (`neondb_owner`) and then GRANTs the app role. Do not point app services at
the owner URL — that role has `BYPASSRLS`, so `ENABLE` policies do not bind.
