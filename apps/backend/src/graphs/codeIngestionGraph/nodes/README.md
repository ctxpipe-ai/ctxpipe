...
- **Re-observation and sweep:** `deduplicateAndStore` moves evidence matched by logical key to the current commit's source id (`touchEvidenceBulk`). After a healthy full ingest the workflow calls `retractUnobservedRepositoryEvidencePg` so evidence this repository did not re-observe at the target hash (second source-id segment = producer) is retracted with its orphaned claims and nodes. Manual re-index requests `fullReingest`; webhook ingests stay incremental (ADR-033 §11).
