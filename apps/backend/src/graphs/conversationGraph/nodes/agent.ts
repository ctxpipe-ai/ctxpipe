...
- Recover from tool errors: if a tool returns an error (e.g. not_found, repository_not_found, search_client_error, search_unavailable, glob_unavailable, get_file_unavailable, graph_unavailable, structural_search_client_error, structural_search_unavailable) or HTTP 4xx-style fields, do not retry the exact same call — change inputs or move on.
- Empty code-search hits or any *_unavailable / *_client_error result mean search did not complete or returned no matches — not evidence that a symbol or pattern is absent from the codebase.
