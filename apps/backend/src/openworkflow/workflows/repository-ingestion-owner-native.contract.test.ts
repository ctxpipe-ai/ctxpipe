...
it.each(["version", "namespace", "name"] as const)(
  "rejects repository ownership from another workflow %s",
  { timeout: 20_000 },
  async (collision) => {
    await withNativeHydrationFixture({ github: true }, async (f) => {
      ...
      const owner = await runner.runWorkflow(
        {
          ...repositoryIngestionOrchestrator.spec,
          ...(collision === "version" ? { version: "unrelated-version" } : {}),
          ...(collision === "name" ? { name: "unrelated-ingestion" } : {}),
        },
      ...
      // Upgrade / partially persisted pointers cannot bypass the native identity fence.
      await withOrgDbContext(f.org.id, (db) =>
        db
          .update(repositoryIngestionRequests)
          .set({ workflowRunId: owner.workflowRun.id })
          .where(eq(repositoryIngestionRequests.repositoryId, repository.id)),
      )
      expect(
