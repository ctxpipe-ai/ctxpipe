...
describe("MCP request attribution", () => {
  it("puts tool and conversation ids on the POST /mcp span and request log before the tool runs", async () => {
      ...
      handleMcpTransportRequest(c, (server) => {
        server.registerTool(
          ...
          async () => {
            toolStarted = true
            const span = trace
              .getTracer("ctxpipe-mcp-tool-test")
              .startSpan("mcp.tool ctx_advisor")
            span.setAttribute("ctxpipe.mcp.tool", "ctx_advisor")
            span.setAttribute(
              "ctxpipe.conversation.id",
              "org_acme_user_1_my-backend_conv-xyz",
            )
            span.end()
    ...
    const toolSpan = exporter
      .getFinishedSpans()
      .find((span) => span.name === "mcp.tool ctx_advisor")
