...
async function withSearchProjection(
  run: (fixture: {
  ...
  if (!databaseUrl) throw new Error("DATABASE_URL is required for search proof")
  const directory = await mkdtemp(join(tmpdir(), "ctxpipe-search-projection-"))
        ...
        Result: {
          Files: [
            ...
            },
            {
              ...
              LineMatches: [{ LineNumber: 1, Line: "Published instructions" }],
            },
          ],
        },
      }),
...
it(
  "search excludes index results ahead of the published revision",
  { timeout: 60_000 },
  async () => {
    await withSearchProjection(async (f) => {
      expect(
        await codeSearch(f.org.id, {
          query: "instructions",
...
it(
  "repository-scoped search serves only its published immutable revision",
  { timeout: 60_000 },
  async () => {
    await withSearchProjection(async (f) => {
      f.publishIndex()
      const matches = await codeSearch(f.org.id, {
        query: "instructions",
        workspaceId: f.workspaceId,
        repositoryIds: [f.repositoryId],
      })
...
it(
  "workspace search includes each linked repository's published SHA",
  { timeout: 60_000 },
  async () => {
    await withSearchProjection(async (f) => {
      f.publishIndex()
      ...
      expect(matches[0]?.response.Files).toEqual(
