...
- **MCP (product)**: Integrate the **product** MCP in-process with the Hono API at `/mcp` using the official `@modelcontextprotocol/sdk` web-standard Streamable HTTP transport. Do **not** run a separate process for that product MCP. Hosted `/mcp` is the cross-client surface ([ADR-029](../../.ai/memory/decisions/ADR-029-hosted-mcp-client-interoperability.md)): accept authenticated GET as a keepalive SSE listener; do not reintroduce a blanket GET 405 to chase conformance scores. This does **not** forbid Cursor **agent tooling** MCPs in [`.cursor/mcp.json`](../../.cursor/mcp.json) (Neon, Amplitude, Railway, Langfuse, Better Stack, Storybook, memory, etc.); those are editor-side helpers, not a second product `/mcp` server.
