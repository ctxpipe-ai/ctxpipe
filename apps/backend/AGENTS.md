...
- **Infra-only**: To run the backend on the host against Compose databases, run `docker compose up -d postgres neo4j`, then set `DATABASE_URL` (and optionally `NEO4J_URI`) and run `pnpm dev` from `apps/backend`. See [.claude/memory/decisions/ADR-004-local-development-docker-compose.md](../../.claude/memory/decisions/ADR-004-local-development-docker-compose.md).
