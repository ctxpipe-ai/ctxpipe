...
- **MCP (product)**: Integrate the **product** MCP into the Hono app via `@hono/mcp` (Streamable HTTP at `/mcp`). Do **not** run a separate process for that product MCP — it stays in-process with the API. This does **not** forbid Cursor **agent tooling** MCPs in [`.cursor/mcp.json`](../../.cursor/mcp.json) (Neon, Railway, Langfuse, Storybook, memory, etc.); those are editor-side helpers, not a second product `/mcp` server.
...
- **Connections (GitHub + Confluence/Forge + Slack + Linear + Notion + PagerDuty)**: Stored in **`connections`** (`con_*` ids, `type` `github` \| `forge` \| `slack` \| `linear` \| `notion` \| `pagerduty`, `config` jsonb). Legacy **`github_installations`** / **`forge_installations`** tables are removed; repos use **`github_connection_id`**, Confluence tables use **`connection_id`**, and Slack/Linear/Notion/PagerDuty each keep their Git-native (or intent-capture) binding in `connections.config`. Do not assume one connector per org — use explicit **`connectionId`**, list endpoints, or resolve via **`repository_id`**. **`GET /:orgSlug/api/v1/connectors`** returns metadata only (no `config`). Designing or adding a connector: **[source-connectors skill](../../.agents/skills/source-connectors/SKILL.md)**. See [.ai/memory/decisions/ADR-018-unified-connections-table.md](../../.ai/memory/decisions/ADR-018-unified-connections-table.md), [ADR-022](../../.ai/memory/decisions/ADR-022-linear-connector-git-native-mirror.md) (Linear), [ADR-023](../../.ai/memory/decisions/ADR-023-notion-connector-git-native-mirror.md) (Notion), [ADR-025](../../.ai/memory/decisions/ADR-025-slack-connector-git-native-mirror.md) (Slack), and [ADR-034](../../.ai/memory/decisions/ADR-034-pagerduty-connector-git-native-mirror.md) (PagerDuty).
