...
- **MCP OAuth `invalid_grant: session not found` (Cursor / Codex)**: In `@better-auth/oauth-provider`, the refresh-token handler returns **`error_description: "session not found"`** when **`findOne` on `oauth_refresh_tokens` returns no row** (after hashing the presented refresh token with `storeTokens: "hashed"`). That string is **not** a Better Auth “user session” lookup failure; it usually means the **stored refresh token row is missing** (DB reset / wrong `DATABASE_URL`, row deleted, **`oauth_clients` removed** and cascade-deleted tokens, or reuse of an already-rotated refresh token). **Mitigation for users**: disconnect the MCP server in the client and complete OAuth again so a new row is written. **Ops**: confirm the app and OAuth callback hit the **same** Postgres; avoid wiping `oauth_*` tables without re-auth.
