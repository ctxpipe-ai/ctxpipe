...
- **Connections (GitHub + Confluence/Forge + Linear + Notion)**: Stored in **`connections`** (`con_*` ids, `type` `github` \| `forge` \| `linear` \| `notion`, `config` jsonb). Legacy **`github_installations`** / **`forge_installations`** tables are removed; repos use **`github_connection_id`**, Confluence tables use **`connection_id`**, and Linear/Notion keep Git-native binding in `connections.config`. Do not assume one connector per org — use explicit **`connectionId`**, list endpoints, or resolve via **`repository_id`**. **`GET /:orgSlug/api/v1/connectors`** returns metadata only (no `config`). Designing or adding a connector: **[source-connectors skill](../../.agents/skills/source-connectors/SKILL.md)**. See [.ai/memory/decisions/ADR-018-unified-connections-table.md](../../.ai/memory/decisions/ADR-018-unified-connections-table.md), [.ai/memory/decisions/ADR-022-linear-connector-git-native-mirror.md](../../.ai/memory/decisions/ADR-022-linear-connector-git-native-mirror.md), and [.ai/memory/decisions/ADR-023-notion-connector-git-native-mirror.md](../../.ai/memory/decisions/ADR-023-notion-connector-git-native-mirror.md). Slack (intent capture) is PR #267.
