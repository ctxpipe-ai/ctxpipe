...
The provider cannot set Serverless. After apply: collector, ClickHouse, and Langfuse worker `sleepApplication=false`; HyperDX/Mongo/Langfuse web/Redis stay Serverless; HyperDX also needs `OTEL_SDK_DISABLED=true` and `RUN_SCHEDULED_TASKS_EXTERNALLY=true` so it does not export to HyperDX Cloud or reconnect Mongo every minute. Langfuse worker is the stock always-on image (`node worker/dist/index.js`).
