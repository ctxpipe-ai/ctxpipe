...
ClickHouse, the collector, and `railway-telemetry` run CI images `ghcr.io/ctxpipe-ai/obs-<svc>:<git tree hash>`, where the tag is `git rev-parse HEAD:ops/observability/<svc>`. [`.github/workflows/observability.yaml`](../../../.github/workflows/observability.yaml) builds and pushes those images, then sets `TF_VAR_collector_image`, `TF_VAR_clickhouse_image`, and `TF_VAR_railway_telemetry_image`. Terraform `source_image` pins them. HyperDX, Langfuse, Redis, and Mongo pull public images.
...
The provider cannot set Serverless. Collector and ClickHouse set `sleepApplication = false` in `railway.toml`. `railway_service.railway_telemetry` sets `cron_schedule = "*/5 * * * *"`; provider 0.6.1 sends `cronSchedule` on every update with no `omitempty`, so an unset attribute would clear the live cron. [`../railway-telemetry/railway.toml`](../railway-telemetry/railway.toml) uses the same schedule and `restartPolicyType = NEVER`. Sleep policy: [../README.md](../README.md#awake-vs-sleep).
...
Create GitHub Environment `observability` with required reviewers and a `main`-only deployment branch policy before merging. If it is missing, the first push creates it with no rules and apply runs ungated.
...
[`imports.tf`](./imports.tf) adopts the API-created services. It is a one-time import. Delete the file after the first successful apply on `main`. Until then, the first plan should show imports and in-place updates. Import blocks do nothing once that address is in state. Railway-owned secrets are omitted from the variable-collection import ids, so they never enter state.
...
5. Open a pull request that touches `ops/observability/**` and review the plan comment. Merge when it is imports plus in-place updates.
