...
- **backend auth context route** – `apps/backend/src/routes/v1/auth.ts` (`GET /v1/auth/me`): authenticated user/session context endpoint that reports the inferred last login method from latest linked auth account provider.
