...
- **Deployment-shared** app credentials live in env (`LINEAR_*`, `SLACK_*`). **Connection-specific** credentials (and provider-issued secrets) live encrypted in `connections.config`. **Notion** is Atlassian-shaped: OAuth client ID/secret and webhook verification token may live on the Notion `connections` row, with `NOTION_*` env as hosted fallback — not “always `NOTION_*` env.” Document which model this provider requires. Backend **and** worker resolve the same way when the worker refreshes tokens.
