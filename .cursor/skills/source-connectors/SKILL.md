...
- **Deployment-shared** app credentials live in env (`LINEAR_*`, `NOTION_*`, `SLACK_*`). **Connection-specific** credentials (and provider-issued secrets) live encrypted in `connections.config`. PagerDuty follows the Confluence split: hosted may use `PAGERDUTY_CLIENT_ID` / `PAGERDUTY_CLIENT_SECRET`; self-host saves the app on the `connections` row and resolves **row, then env**. Linear/Notion stay env-app until a later PR. Backend **and** worker need whichever client secret the refresh path will read.
