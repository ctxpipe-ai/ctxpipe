...
# Uses GraphQL for lookup/delete when the CLI cannot read or remove a broken env.
...
if [[ -n "$ENV_ID" ]]; then
  if env_config_readable "$PR_ENV"; then
    ...
    echo "Railway environment $PR_ENV exists but is not usable; deleting and recreating from $SOURCE_ENV"
    delete_env_by_id "$ENV_ID"
    if ! create_duplicated_env; then
      echo "railway environment new failed after delete; cleaning up so retry can succeed" >&2
  ...
  if ! create_duplicated_env; then
    echo "railway environment new failed; deleting stale environment so retry can succeed" >&2
    delete_pr_env_if_present || true
    exit 1
  fi
fi
