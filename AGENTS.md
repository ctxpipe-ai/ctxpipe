...
- **Setup**: `npx ctxpipe init --memory` writes a `ctxpipe-memory` MCP entry next to `ctxpipe` in client configs (Cursor, Claude, OpenCode, VS Code; Codex manual fallback) and seeds `.ai/memory/README.md` only if absent. Optional `--claude-hooks` installs per-user Claude Code SessionStart/Stop hooks.
- **CLI**: `ctxpipe memory mcp` is the stdio MCP server invoked by agents. `ctxpipe memory status|doctor|stop` is for humans. `ctxpipe memory hook claude-session-start|claude-stop` is for agent-native hooks.
