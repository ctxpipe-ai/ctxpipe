...
3. **`pnpm dev:infra`** — Docker must be running. Starts Postgres, FalkorDB, otel-collector (Compose **`infra`** profile only). For **local self-hosted Langfuse** (LLM trace debugging), see [apps/otel-collector/README.md](apps/otel-collector/README.md).
