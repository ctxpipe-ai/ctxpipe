...
- **Connectors data model**: GitHub, Confluence/Forge, and Slack integrations live in **`connections`** (`con_*`, typed `github` \| `forge` \| `slack`); see [ADR-018](.ai/memory/decisions/ADR-018-unified-connections-table.md) and [ADR-022](.ai/memory/decisions/ADR-022-slack-connector-git-native-mirror.md). Prefer **`connectionId`** or repo-scoped resolution over “one install per org” assumptions.
