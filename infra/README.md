...
- Sets preview-only variables: clears `OTEL_EXPORTER_OTLP_HEADERS`, `AMPLITUDE_REGION`, `ENABLE_LANGSMITH=false` (duplicated OTLP endpoints and `AMPLITUDE_API_KEY` stay as in production); worker `OPENWORKFLOW_IDLE_EXIT_SECONDS`; backend `RAILWAY_TOKEN` when `RAILWAY_ENVIRONMENT_NAME` starts with `pr-` to wake the `openworkflow` service after enqueue
