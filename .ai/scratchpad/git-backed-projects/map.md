...
- [Git-canonical knowledge and deterministic hydrate](issues/02-hydration-contract.md) — **draft, combining rule open:** file-native two-layer graph (links + optional `claims:`); maintenance job (not hydrate) upgrades/repairs and commits; skip-all-malformed; clone-from-declaration; Project-scoped knowledge and indexes; file confidence is a per-signal max; recall weakens via temporality; same-edge merge must increase with more signals (formula TBD).
