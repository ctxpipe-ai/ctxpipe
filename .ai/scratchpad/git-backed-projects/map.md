...
- [Git-canonical knowledge and deterministic hydrate](issues/02-hydration-contract.md) — **partial:** serving Postgres + embeddings + FalkorDB + Zoekt/SCIP are derived; hydrate may embed but not extract; operational tables as recommended except **display name is git-canonical** in the root map front matter; foreign agents must be able to author files (path identity). File-native Q1 still open.
