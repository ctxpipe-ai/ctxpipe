...
- [Git-canonical knowledge and deterministic hydrate](issues/02-hydration-contract.md) — **draft, Q12 open:** file-native knowledge; Postgres success line with retryable embeddings; skip any malformed file (bad `AGENTS.md` keeps last name; bad `repositories/*.md` unlinks); `AGENTS.md` ops agent edits name + marked section only; rename-ref repair in its own commit; `repositories/*.md` authorizes clone (authz may reject; UI error); knowledge and **indexes are Project-scoped**; integrations/auth are shared many-to-many. Generic-link vs rich-claim graph and confidence/temporality still open.
