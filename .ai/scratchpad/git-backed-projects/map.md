...
- **Chat runtime (locked):** TanStack AI `chat()` + `withSandbox(defineSandbox(…))` + `opencodeText`. Isolation is a TanStack sandbox provider (`dockerSandbox` where a Docker daemon exists). Do **not** spawn `opencode serve`, `docker run` OpenCode, or host git worktrees as the chat isolation mechanism. See [Chat uses TanStack sandbox, not DIY OpenCode](issues/17-tanstack-sandbox-not-diy-opencode.md).
...
- [Git-canonical knowledge and deterministic hydrate](issues/02-hydration-contract.md) — **partial:** serving Postgres + embeddings + FalkorDB + Zoekt/SCIP are derived; hydrate may embed but not extract; operational tables as recommended except **display name is git-canonical** in the root map front matter; foreign agents must be able to author files (path identity); **linked repositories are git-canonical** in the backing tree (URL list vs submodules still open). File-native Q1 still open.
