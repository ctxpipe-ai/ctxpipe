...
- [Git-canonical knowledge and deterministic hydrate](issues/02-hydration-contract.md) — files are canonical (path identity, links as relations); Postgres/Falkor/embeddings/Zoekt/SCIP are derived; hydrate may embed but not extract or rewrite `AGENTS.md`; `AGENTS.md` is the folder map + display name; ops update it via unsandboxed TanStack `chat()`; attached remotes are `repositories/*.md`; hydrate replaces one SHA, skips malformed, no-ops the same SHA; git-detected rename rewrites refs on the **write** path.
