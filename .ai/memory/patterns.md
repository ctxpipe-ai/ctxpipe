...
- **Atlassian Confluence config contract**: keep setup prerequisites and scope editing separate in UI, but persist both space scope and sync target through a single backend contract (`GET/POST /:orgSlug/api/v1/connectors/atlassian/config`); enqueue `confluence-sync-content` in OpenWorkflow after save and for Confluence webhooks (incremental mode).
  <!-- @category: pattern -->
- **Slack connector (git-native mirror)**: `connections.type=slack` with encrypted `botTokenEnc` (GitHub-style, not plaintext). One deployment-owned Slack app (`SLACK_CLIENT_*`, `SLACK_SIGNING_SECRET`); Events at `/api/v1/webhook/slack`. Scope via `slack/config.yaml` PR; mirror under `slack/channels/<slug>--<id>/threads/…`. Private channels opt-in (bot invite + listed); no DMs in v1. Freshness SLO ~10 min: mark dirty on events, flush after ~3 min quiet / ~10 min max lag; `slack-sync-flush` uses `step.sleep` when dirty-but-not-ready (do not rely on 5-min idempotency alone).
  <!-- @category: pattern --> <!-- @topic: backend -->
- **Connector config PRs for empty repositories**: use the shared GitHub `createPullRequestWithFiles` path, which initializes an empty repository with `.gitkeep` before branching. Keep this shared across Notion, Confluence, and Slack rather than adding connector-specific initialization. When PR creation terminates without a URL, show an explicit failure and retry action instead of describing it as still running.
  <!-- @category: pattern --> <!-- @topic: backend -->
- **Connection-specific GitHub webhooks on the legacy route**: the canonical payload URL is `/api/v1/webhook/github/:connectionId`. For an existing app still posting to `/api/v1/webhook/github`, first try the deployment-global HMAC; on failure, use the untrusted payload installation id only to select candidate connection secrets, then require a successful candidate HMAC before processing. Log the compatibility path so operators can correct the app URL.
  <!-- @category: pattern --> <!-- @topic: backend -->
- **Preview Neon connections**: use the Neon branch action's direct `db_url` only for Drizzle migrations; set Railway runtime services (`backend`, `openworkflow`, and `codesearch`) to `db_url_pooled`. Durable connector workflows must persist a terminal failure phase and expose an idempotent retry action rather than leaving setup in an indefinite in-progress phase after the workflow run has failed.
  <!-- @category: pattern --> <!-- @topic: backend -->
...
- **UI copy language**: use UK English spelling in user-facing UI copy, for example `organisation` rather than `organization`
  <!-- @category: convention -->
- **Connector setup controls**: keep form fields, repository handoff panels, status panels, and list containers square (`rounded-none` / no radius); preserve the established Confluence and Notion connector treatment rather than inheriting rounded design-system defaults
  <!-- @category: convention -->
