...
| Job | Background work that **updates the workspace repository** (ingest, connector sync, maintenance, ops/bootstrap, link/unlink, rename rewrite, semantic rebase, …). Examples: a reference-link fix; a Confluence webhook that writes a document; a linked-repository merge that processes the change into the Workspace. Distinct from **hydrate**, which refreshes the projection and does not edit git. |
...
| Temporality | Optional `[valid_from, valid_to)` window. Missing `valid_to` = evergreen (source half-life). Missing `valid_from` is derived from the introducing git commit at hydrate, then persisted by a maintenance **job**. `e = 0` before `valid_from` and at/after `valid_to`. |
