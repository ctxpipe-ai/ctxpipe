...
| Temporality | Optional validity window on a claim (`valid_from` / `valid_to`; missing both = evergreen). Hydrate copies from the file. A maintenance job backfills `valid_from` from a git commit timestamp (fill vs bump still open). |
