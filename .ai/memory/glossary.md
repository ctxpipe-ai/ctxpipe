...
| Confidence | Optional 0–1 score on a claim (author-stated; the knowledge skill asks the user). Storage in git vs serving aggregation is still open on the hydrate ticket. |
| Temporality | Optional validity window on a claim (`valid_from` / `valid_to`; missing = evergreen). Storage and query-time filtering still open on the hydrate ticket. |
