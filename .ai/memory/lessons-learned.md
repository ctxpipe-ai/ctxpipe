...
- **Rule:** `repository-ingestion` passes `fromHash = lastIngestedHash` to codesearch; when that commit is an ancestor of the target (including the same commit) the run is `partial`, and `retractIngestionForDiffPg` is a no-op without changed paths while the extractors still re-run over the whole repository. Non-deterministic (LLM) extractors then mint new dedup keys next to the old ones and the object count only grows — TruRec production held `extractInstructionUnits` evidence from 586 distinct ingestion hashes, and the PR #335 preview gained 113 instruction units between two runs at the same tip. Treat "re-index" as `fullReingest: true` (no `fromHash`), and after a healthy full run sweep this repository's evidence whose source id does not end in the target hash (`retractUnobservedRepositoryEvidencePg`); dedup must first move re-observed rows to the new hash (`touchEvidenceBulk`) or the sweep deletes live facts. Never run the sweep on a degraded run (search/SCIP index failed). When judging a graph change, compare object counts across two runs at the same tip: growth means accumulation, not new knowledge.
- **Category:** reliability
- **Date:** 2026-09-17
