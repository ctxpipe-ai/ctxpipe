...
6. **Names.** Ids come from the authenticated context on public services, never from inbound baggage or other client headers. Baggage is added only on fetches to internal origins (codesearch, `*.railway.internal`, localhost). Resource attributes are `service.name` (`backend`, `openworkflow`, `codesearch`, `ui`), `service.namespace=ctxpipe`, and `deployment.environment` (the Railway environment name, or `development` off Railway). ClickHouse materializes `DeploymentEnvironment` from that key. Span and request-log HTTP fields are `http.request.method`, `url.path` (no query string), and `http.response.status_code` for this service's own response. A log about a call this service made uses `upstream.status_code`. Identity keys are `request.id`, `enduser.id`, `ctxpipe.org.id`, `ctxpipe.org.slug`, and `ctxpipe.actor.type` (`user`, `org_api_key`, `oauth_client`, `webhook`, `job`). Other `ctxpipe.*` ids (api key, oauth client, mcp tool, conversation, repository, connection) are set when the request or job has them. The browser sets HyperDX session keys `userId`, `teamId`, and `teamName` together with `enduser.id`, `ctxpipe.org.id`, and `ctxpipe.org.slug`, and posts `POST /.otel/v1/$signal` (traces and logs). Langfuse trace metadata keeps Langfuse's names (`userId`, `sessionId`, `orgId`, `orgSlug`, `requestId`, `environment`, tags `org:<slug>` and `env:<deployment.environment>`). `ctxpipe.org.id` is a metric attribute only on `ctxpipe.advisor.calls`, `ctxpipe.ingestion.jobs`, and `ctxpipe.connector.syncs`. Rows already stored keep the keys they were written with; there is no backfill.
