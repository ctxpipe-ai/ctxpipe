...
2. **evlog for logs**: Use evlog with Hono middleware. When `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` is set, logs drain to OTLP through the custom fetch in `logger.ts`, with evlog's batching and retry pipeline. The POST uses `AbortSignal.timeout(5_000)`. A failed or timed-out export is retried, then dropped; it does not fail the request or shutdown. Otherwise logs go to stdout only.

3. **Single endpoint, collector fan-out**: The app sends to one configurable OTLP endpoint per signal. For multiple backends, users run an OpenTelemetry Collector and configure it to fan out. **Hosted ingest** is the ClickStack collector with a merged custom file ([`ops/observability/collector/config.yaml`](../../../ops/observability/collector/config.yaml)): the **APM** path is the built-in ClickHouse exporter (full traces); the **LLM** path is `filter/llm_only` → Langfuse. [`apps/otel-collector/config.yaml`](../../../apps/otel-collector/config.yaml) is the laptop debug sink (stdout only). No app-side fan-out or LangFuse-specific env in the backend.
...
5. **Register-first on Bun.** [`apps/backend/src/server.ts`](../../../apps/backend/src/server.ts) imports [`observability/register.ts`](../../../apps/backend/src/observability/register.ts) before the app, so `initOtel` runs before `createApp`. The Hono middleware [`backendOtelMiddleware`](../../../apps/backend/src/observability/http.ts) is the server span: it continues `traceparent` / `baggage`, names the span from the matched route, and returns `x-request-id` on every response including 5xx. Proxied UI assets (Vite modules and hashed files) skip that span and still set the header. A `fetch` wrapper creates the client span and injects W3C `traceparent`, because Bun's `fetch` is not undici. Baggage is added only for internal services (see attribution below). OTLP `/v1/{traces,metrics,logs}` URLs are left uninstrumented. Codesearch does not use `NodeSDK`. [`tracerProvider.register()`](../../../apps/codesearch/src/observability/otel.ts) runs before the app, then the same kind of Hono server span and fetch wrapper. `@opentelemetry/sdk-node` auto-instrumentations do not patch `Bun.serve` or Bun's global `fetch`.
   ...
   Export filters sit in front of the batch processor. `BetterAuthSpanFilter` keeps a `better-auth` endpoint span only when its parent is a `/.auth/` server span. Hook, handler, middleware, and adapter `db *` spans are dropped. Postgres failures stay on `dbTrace` (`ctxpipe-backend`). `DropParentlessAutoInstrumentationSpans` drops parentless CLIENT and INTERNAL spans whose instrumentation scope starts with `@opentelemetry/instrumentation-`. A span with ERROR status is kept, so a Redis or `https.request` failure outside a job still exports. SERVER and CONSUMER spans, and spans from `ctxpipe-backend`, are kept. `nodeAutoInstrumentationConfig` disables `instrumentation-dns`, `instrumentation-fs`, `instrumentation-net`, `instrumentation-pg`, and `instrumentation-undici`: those were idle roots (`dns.lookup`, `tcp.connect`, `pg-pool.connect`) or a second client span that replaced the fetch wrapper's `traceparent`. `instrumentation-http` stays for `http.request` / `https.request` and ignores OTLP export URLs.
...
6. **Attribution** ([`attribution.ts`](../../../apps/backend/src/observability/attribution.ts)). One set, ids only. Logs drop email, name, image, IP, and user-agent (`stripLogPii`). The server span may still carry `user_agent.original`.
   ...
   **Names.** One key per concept. Resource attributes are `service.name` (`backend`, `openworkflow`, `codesearch`, `ui`), `service.namespace=ctxpipe`, and `deployment.environment` (`production` | `pr-N` | `observability`). Not `deployment.environment.name`: ClickStack seeds `__hdx_materialized_deployment.environment.name` on `otel_logs` only, from that longer key, and the column stays empty. A materialized column whose name is the prefix of that seed is rejected. `DeploymentEnvironment` is materialized from `deployment.environment` on logs, traces, and metric tables, and the dashboards filter that expression. Do not also emit `.name`.
   ...
   evlog's wide event used to carry `environment`, `service`, `method`, `path`, and `status`. Those are not log attributes anymore. The OTLP resource has `deployment.environment` and `service.name`. HTTP fields on logs match spans: `http.request.method`, `url.path`, `http.response.status_code`. `http.response.status_code` is this service's own response, and the contract copies a top-level `status` onto it only when `url.path` or `http.request.method` is also present. The status of a call this service made (codesearch, Zoekt, GitHub, Bedrock, and the same shape elsewhere) is `upstream.status_code`. A bare `status` without those HTTP fields stays as `status`. `duration` stays the evlog request duration in milliseconds. Trace duration is the `Duration` column (nanoseconds). `traceId` / `spanId` stay on the wide event only long enough for evlog to fill the `TraceId` / `SpanId` columns; they are removed from the JSON body. There is no `trace_id` attribute.
   ...
   `applyAttribution` writes the active span, the request logger, and an in-process bag. A span processor copies that bag onto child spans. Outbound `fetch` to internal services (the configured codesearch URL origin, `*.railway.internal`, and localhost, including `URL.hostname` `[::1]`) carries the keys on W3C baggage. Third-party fetches get `traceparent` only: that header is a trace id and span id, so a peer that understands W3C can join the trace, and it does not include user, org, api-key, or conversation ids. Codesearch reads inbound baggage onto its spans and evlog events, and applies the same internal-only rule when it calls out.

   **Open product decision: Better Auth `dash()`.** `@better-auth/infra`'s `dash()` plugin is enabled in [`apps/backend/src/auth/config.ts`](../../../apps/backend/src/auth/config.ts). It POSTs account events, including sign-in, to `https://dash.better-auth.com/events/track`. A sign-in event includes user id, name, email, session id, user-agent, and IP, city, and country. That host is a third party, so the outbound fetch carries `traceparent` only and carries no baggage. Whether the plugin stays is undecided.
...
- **Filter traces per target**: Initially deferred in favor of “LangFuse filters on ingest.” **Superseded**: OTLP ingest still recorded infra noise; the **ClickStack custom file** applies an **allowlist** on the LLM-bound trace pipeline only, while the APM pipeline stays full-fidelity. **Note (2026-09-26):** that allowlist lives only in the ClickStack file. The laptop collector is a debug sink ([ADR-038](ADR-038-self-hosted-clickstack-langfuse.md)).

### Notes

- See `apps/backend/src/observability/otel.ts`, `register.ts`, `http.ts`, `attribution.ts`, `jobTelemetry.ts`, `businessMetrics.ts`, `evlog.ts`, `langfuse.ts`, [`ops/observability/collector/config.yaml`](../../../ops/observability/collector/config.yaml) (hosted allowlist), and `apps/otel-collector/config.yaml` (laptop debug sink).
