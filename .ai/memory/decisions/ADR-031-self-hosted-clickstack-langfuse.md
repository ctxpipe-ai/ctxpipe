...
- First apply is manual `terraform apply` plus one Railway bucket (`langfuse-events`) the 0.6.1 provider cannot create. DNS for `telemetry.ctxpipe.ai`, `hyperdx.ctxpipe.ai`, and `langfuse.ctxpipe.ai` is operator-owned. The Railway Terraform provider 0.6.1 cannot set Serverless sleep; GitHub-built services set it in `railway.toml`, image services in the Railway service config.
- Langfuse isolation is a second database on the existing Neon project, not a schema.
- Ops-stack Serverless is a flag, not observed idle: `ctxpipe-backend` `POST /v1/metrics` every 60s keeps the collector (and ClickHouse) awake; Langfuse worker `ClickhouseWriter` (1s) plus Redis/Neon pools keep worker, Redis, ClickHouse, and often `langfuse-web` awake; HyperDX’s Mongo pool keeps HyperDX and Mongo awake. Product **PR** apps can still sleep once they run `FlushOnDemandMetricReader` on this branch.
