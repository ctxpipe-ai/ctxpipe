...
**Status:** Accepted | **Date:** 2026-04-22 | **Tags:** database, connectors, github, forge, confluence, slack, multi-tenant
...
1. **One table:** `connections` with `id` prefixed `con_` (via `generateObjectId("con")`), `org_id`, `type` (`github` | `forge` | `slack`; see [ADR-022](ADR-022-slack-connector-git-native-mirror.md) for Slack), `config` (jsonb — type-specific fields; **secrets stay in config** and are never returned on list endpoints), and timestamps. Indexes on `(org_id)` and `(org_id, type)` for listing. **No** global `UNIQUE (org_id, type)`; add targeted uniqueness in `config` or partial indexes when a specific invariant is required.
