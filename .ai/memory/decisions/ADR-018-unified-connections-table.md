...
1. **One table:** `connections` with `id` prefixed `con_` (via `generateObjectId("con")`), `org_id`, `type` (`github` | `forge` | `slack` | `linear` | `notion`), `config` (jsonb — type-specific fields; **secrets stay in config** and are never returned on list endpoints), and timestamps. Slack extends this type set with its intent-capture binding described in [ADR-025](ADR-025-slack-connector-git-native-mirror.md); Linear in [ADR-022](ADR-022-linear-connector-git-native-mirror.md); Notion in [ADR-023](ADR-023-notion-connector-git-native-mirror.md). Indexes on `(org_id)` and `(org_id, type)` for listing. **No** global `UNIQUE (org_id, type)`; add targeted uniqueness in `config` or partial indexes when a specific invariant is required.
