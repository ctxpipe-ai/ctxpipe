...
**Status:** Accepted | **Date:** 2026-04-22 | **Tags:** database, connectors, github, forge, confluence, slack, linear, notion, multi-tenant
...
1. **One table:** `connections` with `id` prefixed `con_` (via `generateObjectId("con")`), `org_id`, `type` (`github` | `forge` | `slack` | `linear` | `notion` | `pagerduty`), `config` (jsonb — type-specific fields; **secrets stay in config** and are never returned on list endpoints), and timestamps. Slack extends this type set with its intent-capture binding described in [ADR-025](ADR-025-slack-connector-git-native-mirror.md); Linear in [ADR-022](ADR-022-linear-connector-git-native-mirror.md); Notion in [ADR-023](ADR-023-notion-connector-git-native-mirror.md); PagerDuty in [ADR-030](ADR-030-pagerduty-connector-git-native-mirror.md). Indexes on `(org_id)` and `(org_id, type)` for listing. **No** global `UNIQUE (org_id, type)`; add targeted uniqueness in `config` or partial indexes when a specific invariant is required.

2. **Dependents:** Repository rows reference the GitHub connection via **`github_connection_id`** → `connections.id`. Confluence **`confluence_spaces`** and **`confluence_sync_targets`** reference **`connection_id`** → `connections.id` (legacy table). **New git-native connectors** (Linear, Notion, Slack) store the context-repo binding on **`connections.config`** jsonb (`repositoryId`, `branch`, `enabled`) rather than a per-connector sync-targets table; Confluence remains the table exception. Bindings are unique per connection because they live on the connection row. Do not store a free-text repository name as the binding.
