...
- Claude Code hooks observe **`UserPromptSubmit` only** (plus Stop finalize). `PostToolUse` is not installed; leftover tool-sourced pending ids are dismissed on summarize. `npx ctxpipe memory init` strips a leftover Claude `PostToolUse` capture hook on re-init.
- Claude Code Stop continuation is top-level **`decision: "block"` + `reason`**. `hookSpecificOutput.additionalContext` works on a fresh 2.1.163+ process (docs + 2.1.251 binary schema) but is **rejected by older CLIs and stale long-lived sessions**. A validation failure is non-blocking: the turn ends and the agent never sees the follow-up. Codex uses the same `decision: "block"` contract. See [stop-output-contract.md](../research/claude-hooks/stop-output-contract.md).
- Claude/Codex Stop follow-ups are **one-shot** (never-shown candidates only). Re-emitting `decision: "block"` for already-surfaced ids would force a continuation on every later turn until promote/dismiss. Cursor already one-shot via `followup_message` + `loop_count`.
