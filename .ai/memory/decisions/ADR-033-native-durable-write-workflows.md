...
- Extraction captures source objects, completed path assignments and migration cutover in one short repeatable-read PostgreSQL transaction. Completed assignment maps are folded across results so an omitted object cannot lose its prior path.
- Overlapping text conflicts use an OpenWorkflow-owned TanStack provider resource with explicit create, resolution and destruction steps. A bounded structured model result may replace only the conflict paths; native Git retains clean merged changes and the broker alone can publish. Mechanical transforms do not share this resource or a registry. Docker resources have a stable native container name via the pinned provider patch, allowing replay to find creation whose acknowledgement was lost.
