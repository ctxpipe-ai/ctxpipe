...
**Status:** Accepted | **Date:** 2026-08-05 | **Updated:** 2026-08-19 | **Tags:** connectors, slack, github, multi-tenant, mcp
...
6. **Layout:** Thread-first Markdown under `slack/channels/<slug>--<channelId>/threads/…` (reuse path layout so ingest/ACL stay consistent). Attachments are **link stubs only** (prefer Slack `permalink`; never persist `url_private`). `files:read` remains for file metadata on thread messages. Thread snapshots cap at **500 messages** (oldest first) so a mega-thread cannot exhaust the shared hosted Slack app quota.
7. **Access prerequisites:** The bot must be invited into any channel where capture is requested. Private channels work when the bot is present and `groups:history` is granted. **No DMs/MPIMs in v1.** Any member of a channel the bot is in may trigger the agent; there is no Slack↔ctx| user mapping in v1. Setup UI shows the **installed bot handle**, not a hardcoded `@ctxpipe`.
